...
async function main() {
  try {
    ...
    const createComputeInstanceParams = [computeClient, availabilityDomains];

    console.log(
      `Running runCreateComputeInstanceInterval with ${RETRY_WAIT_SECONDS}s interval`,
    );
    runCreateComputeInstanceInterval(
      createComputeInstanceParams,
      RETRY_WAIT_SECONDS,
    );
  } catch (err) {
    throw new Error(err);
  ...
  try {
    ...
    for (const ad of availabilityDomains) {
      ...
      try {
        ...
        console.log(
          `[SUCCESS] Instance created successfully in ${ad.name}:`,
...
async function runCreateComputeInstanceInterval(
  createComputeInstanceProps,
  interval,
) {
  for (;;) {
